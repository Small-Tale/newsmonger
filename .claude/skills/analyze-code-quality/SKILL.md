...
npm run test:e2e 2>&1   # or `npm run test:all` for the full gate
```
...
- switches on an internal `mode` / `type` / `status` / `phase` / `kind` field (e.g. `queryItems`'s review-vs-normal modes in `src/db/store.ts`, the schedule modes in `src/checks.ts`, topic sort modes in `src/client/topic-sort.ts`, the settings dialog's tab state);
...
- is a **cache with a fallback path** (e.g. live model discovery falling back to the static list in `src/ai/model-list.ts`, the `AUTO_ORDER` provider resolution in `ProviderResolver`, the image cache's refetch-on-miss in `src/routes/api.ts`, `DiscoveryService`'s suggestion cache);
- holds a **reactive store / signal** whose actions mutate a collection (`src/client/stores.ts`), or drives DOM that must survive kerf's morph (`each()` containers, `data-key` rows — see `docs/3-ui.md`);
- accumulates across a session (the undo buffer in `src/undo.ts`, a topic's `consecutiveFailures`/`retryAfter` cooldown, story threading's pairwise accumulation in `src/threads.ts`, the run history).
