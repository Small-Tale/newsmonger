...
[`domotion-svg`](https://www.npmjs.com/package/domotion-svg), pinned in `devDependencies` (**caret-ranged here, `^0.21.1`, not exact as in glassbox** — so a
minor bump can arrive on any `npm install`; check `node_modules` for what is
actually installed). A domotion release can fix rendering,
...
2. **Bump the pin + install.** Edit `package.json` to the new exact version (keep
   it exact — no `^`; the exact pin is deliberate, so a domotion default-mode
   change can't silently alter the output), then `npm install` to refresh
   `package-lock.json`. Confirm with `grep '"version"' node_modules/domotion-svg/package.json`.
