...
description: Generate a diff-grounded, one-page technical changelog for a Newsmonger release — from the actual code changes between the last production tag and HEAD (the next, still-unreleased version). Asks for the next version number, since package.json holds the last release.
...
## Project-specific traps (read these before writing)

- **The requirements docs (the numbered `docs/` files, `docs/ai/*.md`, `CLAUDE.md`) describe the
...
2. **Run the analysis script** — it does the deterministic git work:
   ...
   **added/removed** files and candidate **new subsystems**, and surfaces the Newsmonger
