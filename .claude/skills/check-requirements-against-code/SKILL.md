...
4. **Synchronize the AI summaries** (`docs/ai/code-summary.md` and `docs/ai/requirements-summary.md`):
   ...
   - For `code-summary.md`: walk through each section and confirm it matches what's actually in `src/`, `src-tauri/`, `scripts/`, and the database schema. Look specifically for:
     ...
     - Database tables/columns that don't match `TABLES`/`MIGRATIONS` in `src/db/sqlite.ts` and the zod shapes in `src/db/schemas.ts`
     - External deps that don't match `package.json` `dependencies` and `scripts/build-sidecar.sh`
     - Any entry in the "Where do I look to…" reverse index that points at a renamed or moved file
   - For `requirements-summary.md`: confirm every requirements doc in `docs/` has a section, the section number matches the doc number, and the status marker (Shipped / Partial / Design only / Deferred / Removed / Rejected / Decided) still reflects reality based on what you found in step 2.
   - **`requirements-summary.md`'s status block is GENERATED in this project** (NEWS-296). Record status in the FR doc and run `npm run docs:requirements` — never hand-edit that block; `tests/unit/requirements-summary.test.ts` fails the gate when it is stale. Its *notes* region is hand-written and is the point of the file. `code-summary.md` is hand-written throughout.
...
5. **Verify the CLAUDE.md index is complete**:
   - Read `CLAUDE.md`. **This project does not keep a per-doc index** — it states the numbered range (currently 1-33) and delegates the per-doc detail to `docs/ai/requirements-summary.md`. So check the *range* is right, that the AI-summaries block is accurate, and that any doc named inline still exists. Do not invent an index; the summary is the index.
