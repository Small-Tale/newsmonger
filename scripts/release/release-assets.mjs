...
//
// Ported from ~/Documents/glassbox/scripts/release/release-assets.mjs. The
// release-desktop.yml workflow consumes it in two places:
...
//
// Keeping both halves here rather than duplicated inline in YAML is what stops
// the bug class this guards against: a download link in the release notes that
// 404s because it doesn't match the filename actually shipped.
// `tests/unit/release-assets.test.ts` pins that the links and the renamed asset
// names always agree.
//
// Plain ESM (.mjs) on purpose: the create-release job runs it with the runner's
// preinstalled `node`, with no npm install or tsx step.

// Only .dmg files are safe to rename. The updater's latest.json references the
// .exe / .msi / .deb / .AppImage / .rpm assets by their original filenames, and
// macOS updates use the .app.tar.gz instead — so the .dmg can be given a
// human-friendly name without breaking auto-update.
//
...
function rawBundleNames(version) {
  return {
    macArm: `Newsmonger_${version}_aarch64.dmg`,
    macIntel: `Newsmonger_${version}_x64.dmg`,
    linuxDeb: `Newsmonger_${version}_amd64.deb`,
    linuxAppImage: `Newsmonger_${version}_amd64.AppImage`,
    linuxRpm: `Newsmonger-${version}-1.x86_64.rpm`,
    winExe: `Newsmonger_${version}_x64-setup.exe`,
    winMsi: `Newsmonger_${version}_x64_en-US.msi`,
  };
}
 ...
 * @param {string} version e.g. "0.1.0"
 * @param {string} repo e.g. "Small-Tale/newsmonger"
...
export function downloadSection(version, repo, tag) {
  ...
  lines.push(
    ...
    '',
    '---',
    `**npm:** \`npm install -g newsmonger@${version}\` — works everywhere Node 22.5+ runs, but lacks the native desktop window the installable builds ship with.`,
  );
  return lines.join('\n');
}
