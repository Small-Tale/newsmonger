...
/**
 * Deterministic news service for tests and offline development (`--ai-test`).
 *
 * Returns the same two stories for a topic on every call, so a second check
 * exercises the dedupe path (nothing new should be added). Topics whose name
 * contains "empty" return no stories; topics containing "fail" throw.
 */
export class MockNewsService implements NewsService {
  ...
  checkTopic(topicName: string, known: KnownItem[], sinceIso: string | null): Promise<FoundNewsItem[]> {
    this.calls.push({ topicName, known, sinceIso });
    const lower = topicName.toLowerCase();
    if (lower.includes('fail')) {
      return Promise.reject(new Error('mock news service failure'));
    }
