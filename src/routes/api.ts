...
/**
 * How long to leave a failed refetch alone before trying it again (NEWS-341).
 *
 * Long enough that a broken `<img>` — which the browser retries on every
 * render — costs one request rather than one per repaint, and short enough that
 * a *transient* failure heals on its own.
 *
 * The first version of this had no expiry at all: one attempt per image per
 * process, on the reasoning that "a restart is a cheap way to ask again". It
 * isn't. This is a desktop app someone leaves open for days, and the first
 * report back was an image that stayed broken because a single slow response
 * from a CDN had permanently poisoned it. A negative cache with no way out is a
 * bug, not a safeguard — the failure it is protecting against is cheap, and the
 * one it causes is invisible.
 */
const REPAIR_RETRY_AFTER_MS = 5 * 60_000;

/**
 * When each image was last attempted, so a failure is retried rather than final.
 *
 * Keyed by **data directory and hash**, not hash alone. The same hash under a
 * different data dir is a different file with its own presence or absence, and
 * one `--data-dir` must not be able to suppress a repair in another.
 *
 * Deliberately not persisted: a URL that fails today may work tomorrow, and a
 * durable "don't try" record would need something to invalidate it. A successful
 * refetch writes a file the route finds before it ever reaches here, so entries
 * for images that healed are simply never consulted again.
 */
const repairAttempted = new Map<string, number>();

/**
 * Refetch a cached image whose file has gone missing, returning its bytes.
 *
 * `null` when there is nothing to repair: no story references this hash, the
 * URL is unsafe or unreachable, or it has already been tried this run.
 */
async function repairCachedImage(
  store: Store,
  dataDir: string,
  hash: string,
  refetch: ((imageUrl: string, dataDir: string) => Promise<unknown>) | null,
): Promise<Buffer | null> {
  if (refetch === null) return null;
  const attemptKey = `${dataDir}\0${hash}`;
  const lastAttempt = repairAttempted.get(attemptKey);
  if (lastAttempt !== undefined && Date.now() - lastAttempt < REPAIR_RETRY_AFTER_MS) return null;
  repairAttempted.set(attemptKey, Date.now());

  // Only a URL a stored story actually carries. This is what keeps the route
  // from being an open proxy: an id nobody references resolves to nothing and
  // no request is made, exactly as when it never fetched at all.
  const sourceUrl = store.imageSourceUrl(hash);
  if (sourceUrl === null) return null;
  if ((await refetch(sourceUrl, dataDir)) === null) return null;
  try {
    return fs.readFileSync(cachedImagePath(dataDir, hash));
  } catch {
    return null;
  }
}
...
export function registerApi(app: Hono<AppEnv>, resolveProvider?: ProviderResolver): void {
  app.get('/api/state', (c) => {
    ...
    // Re-assert the caller's open dialog, if any (NEWS-366). Carried on the poll
    // rather than given its own endpoint — the opposite of `/api/foreground`,
    // and deliberately: a hold only ever *delays* a check, so a stray request
    // asserting one costs a minute, where a forged foreground signal would
    // permit spending someone's subscription quota. Riding the poll also means
    // there is no release to send and none to forget — the hold lapses on its
    // own the moment the client stops naming the topic.
    const holding = c.req.query('holding');
    if (holding !== undefined && holding !== '') c.get('holds').hold(holding);
   ...
   *
   * Dismissing says "I have read this", never "delete that copy" — the backup
   * the notice names is untouched, and is the only route back to the data.
   * Idempotent, so a double-click or a retried request is not an error.
   */
  app.post('/api/quarantine/dismiss', (c) => {
    c.get('store').dismissQuarantine();
    return c.json({ ok: true });
  });

  /**
   * Databases FR-4.9 set aside, and what each one holds (FR-33.1, NEWS-342).
   *
   * Reads the data directory rather than the FR-4.17 notice: that notice is
   * dismissible, and a route back that a dismissal destroys is not a route back.
   * It also finds files left by builds that predate the notice.
   *
   * Usually an empty list, and cheap when it is — the inspection only happens
   * for files that are actually there.
   */
  app.get('/api/recover/candidates', (c) => {
    const resp: SetAsideResp = { databases: listSetAside(c.get('dataDir')) };
    return c.json(resp);
  });

  /**
   * Replace everything in the live database with a set-aside one (FR-33.4).
   *
   * The current contents are written to `pre-recover-<ts>.json` first, so this
   * is not the second irreversible step in a story that began with one.
   */
  app.post('/api/recover', async (c) => {
    const body = await parseBody(c, RecoverReqSchema);
    if (body === null) return c.json({ error: 'bad request' }, 400);
    // The name is validated as a *name*, which is also what makes the path join
    // safe — it can never contain a separator or a `..`.
    if (!isSetAsideName(body.file)) return c.json({ error: 'no such set-aside database' }, 404);
    if (!fs.existsSync(path.join(c.get('dataDir'), body.file))) {
      return c.json({ error: 'no such set-aside database' }, 404);
    }
    try {
      const resp: RecoverResp = recoverSetAside(c.get('store'), body.file);
      return c.json(resp);
    } catch (err) {
      // Still unreadable, or unreadable in a new way. Reported as-is: the
      // reason is the only thing that tells someone whether to try again.
   ...
   */
  app.get('/api/image/:hash', async (c) => {
    ...
    try {
      ...
      // Missing file, live row: refetch it (NEWS-341). A story keeps the URL its
      // image came from, and the cache is content-addressed by that URL, so the
      // refetch lands at this same hash and the row needs no correction.
      //
      // The prune that caused this can no longer empty a whole cache, but the
      // damage it did is already on disk in existing installs — and a file can
      // go missing for duller reasons too (a half-restored backup, a disk tidy).
      // Repairing on demand costs a fetch only for images someone is looking at.
      const refetched = await repairCachedImage(c.get('store'), dataDir, hash, c.get('refetchImage'));
      if (refetched === null) return c.json({ error: 'no such image' }, 404);
      bytes = refetched;
    }
