...
- **FR-11.3** *(Shipped)* Every story card has a share button. It shares a formatted block — **title, summary, and the first source link** (`shareText` in `src/client/share.ts`). It tries the OS share sheet first (`navigator.share`); where that is absent — most desktop browsers, and potentially the Tauri WKWebView, which no-ops `navigator.share` the same way it no-ops `window.confirm` (see [3 — UI](3-ui.md)) — it falls back to copying the block to the clipboard and shows a transient toast to confirm. A share sheet the user *cancels* (AbortError) does nothing and shows no toast; the OS sheet is its own feedback, so the toast is reserved for the clipboard fallback and outright failures.
