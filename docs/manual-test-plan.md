...
4. **Export a file (NEWS-157)**: Settings → Data → click each export link. Each must save a file via the system browser (`<a download>` is a no-op in the WKWebView — this is why the click is routed through `/api/open-external`). Automated in a browser and with a simulated `window.__TAURI__` — **both exports, since NEWS-401**: that automation covered only the stories export, and the topics export shipped dead beside it on the wrong hook. The real webview's download behaviour is still what this step confirms.
