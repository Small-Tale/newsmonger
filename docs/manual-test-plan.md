...
5. **Click the OS notification and record what happens** — this is an open question, not a check with a known answer (NEWS-261). Our JS `onclick` definitely cannot run on the desktop (the shim never reads it), but macOS normally activates the app that *posted* a notification when its contents are clicked, and the posting bundle id is faked to ours — so the window may come forward anyway, without any code of ours. Nothing in our stack does it: mac-notification-sys's delegate only reports the activation type, and the plugin discards it. Whichever way it goes, note it on NEWS-261; it decides whether that ticket is worth anything.
