...
- **FR-27.10** *(Shipped, NEWS-252)* **Restore is a workflow in the app, not a file-moving ritual.** Settings → Data shows what the configured backup folder holds — *"Backup found — 12 topics and 340 stories, saved 3 hours ago"* — and a **Restore from backup** button beside it.
  ...
  **Settings used to advertise the trap this requirement replaced** *(NEWS-309)*. The Data tab still told the reader to put `newsmonger-backup.json` in an empty data folder as `data.json` and start the app — the legacy importer (FR-4.8a), which runs **only into an empty database**. Verified rather than assumed: dropping the file into a folder the app has already opened imports nothing at all, silently, and that is everyone doing a restore, since you have to run the app to find out you need one. The copy now points at the button.
...
- **FR-27.11** *(Shipped, NEWS-255; renamed NEWS-328)* **"Delete all stories" deletes stories and nothing else.** (It was "Clear all stories" until NEWS-328 put a second control beside it — see [31 — Deleting in Bulk](31-bulk-delete.md).) Settings → Data, in its own **Reset** group at the foot of the tab. Topics, settings and API keys all survive — the label and the confirmation both say so, because "clear data" sitting beside a backup control reads like a factory reset, and the fear it has to answer is *"am I about to lose my topics too"*.

  **It is not in the Backup group** *(NEWS-304)*. It shipped there, wedged between the "point this at an iCloud Drive…" paragraph and the "the database stays on this machine" one — two paragraphs about backup with an irreversible delete between them — and styled `class="btn"`, identical to `Back up now` directly above it. That cost in both directions: nobody scans a **BACKUP** heading for *how do I wipe my stories*, and anyone reading about backup met a destructive button whose meaning had to come entirely from its own four words.

  **"Reset", not "Danger zone".** The group holds one action and that action explicitly keeps your topics, settings and keys; the `danger` button variant carries the warning, and the eyebrow carries the findability. A heading that overstates is a heading people learn to skip.

  **Restore stays in Backup** — the other candidate for this group, and the wrong one. Its copy announces a *specific* backup ("Backup found — 3 topics and 41 stories, saved 2 hours ago") produced by the folder field directly above it. Separated from that field the announcement points at nothing. It is also recoverable in a way a clear is not: a restore writes `pre-restore-*.json` into the data folder first (FR-27.10), while a clear has no undo at all — `ClearUndoBuffer` holds 8 topics, so a bulk undo would be partial. Reset holds exactly the actions with no source and no way back.
...
- **FR-27.4** *(Shipped)* Three exits: **"Keep backups here"** (saves and immediately writes a first snapshot), **"Not now"** (re-asks after **one day**), and **"Don't ask again"** (permanent). Both dismissals are **settings**, not `localStorage` — "stop asking me" is a promise, and one kept only per-browser is not kept. Saving with an empty folder is refused rather than treated as an answer: it would close the dialog having changed nothing and, since the offer only fires once, quietly never ask again.
  ...
  **Saving the folder and writing the first snapshot fail independently, and each says so** *(NEWS-312)*. A folder can be accepted and then refuse the write — an unmounted drive, a sync client that owns the directory, a full disk — and the server answers `500`. The client's `.then(onOk, onErr)` had its rejection handler on `updateBackupDir` only, which does **not** cover the promise `onOk` returns, so a failed first backup was an unhandled rejection and the user saw *nothing at all*. Two handlers now, with two messages, because "couldn't save that folder" and "saved, but the first backup failed" send someone to different places. Guarded by `tests/unit/unhandled-rejection.test.ts`, which reads every client source for that shape — lint cannot see it, since the outer `void` satisfies `no-floating-promises` and the inner promise is *returned* rather than floating.
...
- **FR-27.12** *(Shipped, NEWS-237)* A typed folder is **resolved server-side before it is stored**, so what Settings reads back is what will actually be written to. A leading `~` is expanded, whitespace-only means off, and a **relative path is refused** rather than resolved.

  **This was a second FR-27.10 until NEWS-302** — the same id as the restore workflow above. It moved rather than the restore one, which is the more widely cited. Not to 27.11 as the ticket proposed: that is the story clear (NEWS-255).
