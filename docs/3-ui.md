...
Topic and taxonomy activity summaries are specified in [37 — Topic Pulse](37-topic-pulse.md). They preserve this hierarchy: tiny comparable rail sparklines, a shallow summary for one soloed topic or active taxonomy filter, and a deliberate drill-down; the feed remains the primary surface.
...
- **FR-3.1** *(Shipped)* The wordmark (FR-3.58), a global check-interval selector (presets 1h / 3h / 6h / 12h / 1d / 2d / 1w), and a "Check all now" button (disabled while any check runs). **Only the wordmark is in the header now** — this described the pre-NEWS-118 single-column layout. The interval selector lives on Settings → Schedule (FR-3.45) and in onboarding; "Check all now" is on the Watching rail's header. The header carries the sidebar toggle, the wordmark, search and the saved filter.
- **FR-3.1a** *(Shipped)* The Source block — **on Settings → Source since NEWS-118, not at the top of the Watching rail** as this used to say: an AI-provider picker (Auto / Claude subscription / ChatGPT subscription / Anthropic / OpenAI / Mock — the two CLI providers postdate the original list), a model field (shown for non-auto/mock), and an endpoint field (shown for OpenAI), persisted via `PATCH /api/settings`. A status line shows the selected provider's availability (from `GET /api/providers`, probed on demand) and the provider that ran the last check. See [6 — AI Providers](6-providers.md).
...
- **FR-3.3** *(Shipped)* Feed: news items across all topics, newest first, **grouped by local calendar day** with Today / Yesterday / "Mon D" headers. Each item has a topic tag, relative found-time, serif title + summary, and source links (prefixed with the outlet's favicon where one resolved, else the arrow glyph — FR-8.14) opening in a new tab (`rel="noopener noreferrer"`). Items animate in on first render (respecting `prefers-reduced-motion`).
- **FR-3.4** *(Shipped)* Errors from user actions appear in an error banner; the most recent failed check (when no action error is showing) appears in a warning banner naming the topic and error.
- **FR-3.5** *(Shipped)* The client polls `/api/state` every 4 s while the tab is visible, so scheduled-check results appear without a reload. It also refreshes **on becoming visible again**, so returning to the app does not show what was true when you left it until the next tick (NEWS-238). The scheduling lives in `src/client/poll.ts` with its dependencies injected, because a rule about *when* to ask cannot be tested through a real 4-second timer.
...
- **FR-3.6** *(Shipped, revised NEWS-433)* Empty states fill the **main content area**, not just the sidebar. The feed panel used to render *nothing at all* when there were no topics — a bare void beside a sidebar that carried the only message, which reads as a broken app, most of all to someone who took the wizard's Skip. Now the panel shows a centred hero (`.feed-empty`), and it does the most in the case the app is worst at:
  - **No topics** — what the app does in a sentence, the **setup guide** (a button that reopens the first-run wizard, otherwise reachable only through Settings → FR-20.3), a **Discover topics** button, and a row of **popular topics as one-click adds** (the same `STARTER_TOPICS` the wizard offers, wired to a plain `addTopic`). A feed with something in it beats an argument for why the wizard should have been finished.
  - **Topics but no stories** — reassurance that the next check will fill the panel, a **Check all now** button to run one immediately, and the setup guide still within reach.
  - **A sub-filter that matched nothing** (search / saved / review) — the plain one-line message it always had. A hero here would shout at a routine empty result and bury the one fact that matters, which is that nothing matched *this* filter. `stories.spec.ts` still asserts the search line, unchanged.

  All of it renders inside the stable `.empty-slot` wrapper, so appearance and disappearance can't disturb the sidebar's keyed topic list — the last remaining `each()` in the app (the KF-377 concern is fixed as of kerfjs 3.0.0, but the stable slot is ordinary good structure now, per the same reasoning as NEWS-99). Guarded by `tests/e2e/empty-state.spec.ts`: the no-topics hero offers the guide and the chips, the guide button reopens the wizard, a chip adds a topic and clears the hero, and the no-stories panel keeps both the check and the guide in reach.
...
- **FR-3.80** *(Shipped, NEWS-458)* When exactly one topic is soloed, plainly clicking a different topic transfers solo to that topic. Modified/range selection clicks do not transfer it, and a multi-topic solo remains additive. Double-click retains its existing solo-toggle semantics.
- **FR-3.81** *(Shipped, NEWS-464)* Solo owns topic scope while one or more topics are soloed: the category row remains visible but disabled, the subcategory row and taxonomy pulse are hidden, and category/subcategory constraints are omitted from feed and pulse requests. The selected taxonomy is preserved unchanged and becomes active again when Solo ends.
