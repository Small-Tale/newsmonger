...
- **FR-37.4** *(Shipped, NEWS-453; refined NEWS-465)* The compact pulse is a reading aid, not a dashboard takeover. It leaves the story feed visible and provides one explicit, text-only **Explore topic pulse** action into the detailed view. The action carries no decorative icon whose alignment can drift when the button wraps or changes width.
- **FR-37.5** *(Shipped, NEWS-453; refined NEWS-457)* An active top-level category or subcategory filter shows a restrained, full-content-width rollup under the taxonomy bar: stored-story volume, change from the previous equivalent period, and a sparkline. The story-count and time-window phrases never wrap internally. A zero-story previous period is labeled **New activity** when the current period has stories and **No activity** when it does not, rather than implying that the stories themselves were compared for similarity. Opening the rollup uses the same detailed view as a topic.
