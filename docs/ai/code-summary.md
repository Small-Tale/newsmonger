...
| Release credentials (certs, secrets, GitHub Actions names) | `docs/5-desktop-app.md` → "What still needs a human — the full credential recipe" and "GitHub Actions secrets". Generation steps, the exact Tauri v2 env var names, the secret table, and production-vs-beta. **Nothing is set up yet** — no signing identity on the dev machine, and `.github/workflows/ci.yml` is gates only (no release job). Two traps documented there: `APPLE_API_KEY` is a Key *ID* not the `.p8`, and `APPLE_API_KEY_PATH` is a path so CI must materialize the file from a base64 secret. See NEWS-21 |
