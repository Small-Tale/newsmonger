...
src/
  ...
  backup-locations.ts suggestedBackupLocations — probes iCloud/Drive/OneDrive/Dropbox per OS, incl. the macOS CloudStorage prefix scan (NEWS-230)
...
| AI effort / how hard the model works | `EFFORT_LEVELS`/`EFFORT_LABELS`/`Effort` in `src/ai/types.ts`; `effort` in `SettingsSchema`; `RunOptions.effort` → `output_config.effort` in `anthropic.ts` (**checks only** — Haiku 4.5 rejects it, so discovery must never inherit it); `<select data-action=effort>` in the Source block, disabled off Anthropic. `--effort` / `NEWSMONGER_EFFORT`. NEWS-189, FR-6.13 | Per-run recording: `runs.effort` column (SCHEMA_VERSION 4), read off `provider.effort` not settings; `null` = not recorded vs `''` = model default. NEWS-226
