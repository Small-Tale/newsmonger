...
| AI effort / how hard the model works | `EFFORT_LEVELS`/`EFFORT_LABELS`/`Effort` in `src/ai/types.ts`; `EFFORT_PROVIDERS`/`providerTakesEffort` is the single list of who takes one — **anthropic, claude-cli, codex-cli** (NEWS-239, NEWS-244); `openai` is a gap, not a decision. `effort` in `SettingsSchema`; `RunOptions.effort` → `output_config.effort` in `anthropic.ts`, `--effort` in `claude-cli.ts`, `-c model_reasoning_effort=` in `codex-cli.ts`. **Checks only** everywhere — Haiku 4.5 rejects it, so discovery must never inherit it. `<select data-action=effort>` in the Source block, disabled only off the four. `--effort` / `NEWSMONGER_EFFORT`. FR-6.13 | Per-run recording: `runs.effort` column (SCHEMA_VERSION 4), read off `provider.effort` not settings; `null` = not recorded vs `''` = model default. NEWS-226
