...
| Cutting a release (the scripts) | `npm run release` / `release:beta` → `scripts/release.sh` (interactive, resumable via `.release-state.json`, `--beta`); `release:beta:auto` → `scripts/release-beta-auto.sh` (non-interactive, `--dry-run`). Notes drafted by **gitgist** over `<lastVersionTag>..HEAD` using `--match 'v*'` (this repo has non-version tags). **`scripts/set-version.mjs`** writes the three files `npm version` misses — `tauri.conf.json`, `Cargo.toml`, **`Cargo.lock`**. Gated by `tests/unit/release-scripts.test.ts`. Diverges from hotsheet/glassbox in four documented ways (see the header of `release.sh`) — notably **beta bumps version files too**, because the workflow guards tag-vs-config and nothing bumps in CI. NEWS-194 |
| Release workflow (build → sign → notarize → verify → publish) | `.github/workflows/release.yml` (NEWS-190). `macos-latest` (required — `codesign`/`notarytool`/`stapler`). **`workflow_dispatch` = dry run, no inputs by design**; `push: tags v*` publishes, prerelease derived from a hyphen in the tag. Guards that the tag's base version matches `tauri.conf.json` + `package.json`. **Passes no `--target`** — `scripts/verify-signing.sh` reads `src-tauri/target/release/bundle/…` and an explicit target would move it. Tauri creates/imports/cleans the keychain itself; do not add `security create-keychain`. Procedure in `docs/5-desktop-app.md` → "Cutting a release" |
