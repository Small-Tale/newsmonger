...
## [27 — The Briefing Reel](../27-briefing-reel.md) — Design decided, nothing built

The day's stories rendered as designed cards in one self-contained animated SVG via `domotion-svg`, optionally narrated and optionally exported as video. Feasibility proven by the NEWS-165 spike (5-scene 1080×1920 reel in ~10.7 s); no implementation yet.

- FR-27.4–27.7 **attribution posture (Decided, NEWS-172)** — story cards **carry the publisher's photo**; the **outlet is a first-class element**, not a footnote; the **AI disclosure sits once on the closing card**, not per card (a single screenshotted card therefore carries none — accepted, revisit if single-card sharing ships); the reel closes on a wordmark card.
- FR-27.17–27.24 **card design (NEWS-166)** — five scenes authored in `src/briefing/cards/` as HTML/CSS; **the design lives in the stylesheet, not the generator**. App tokens only, dark default. The attribution rail is **pinned bottom** on every card (a rail that moved would flicker in motion), with a second auto margin balancing the copy — the first pass left a dead band of several hundred px. Long headline **steps down**, deck clamps. No-photo is a designed card, not a hole. **Vertical only** — square/landscape are NEWS-176/177, and are not rescales.
- FR-27.1–27.3 reel shape, export-derived scope rules (reuses FR-21.1/21.2), formats **vertical first** (`reel` 1080×1920), square/landscape to follow: **Design only**
- FR-27.8–27.10 capture pages may reference images **only** via the local cache or the browser silently breaks FR-8.4; downscale before embedding (photos were ~535 KB of a 611 KB reel); a missing image is the normal case (FR-8.2): **Design only**
- FR-27.11–27.13 **audio is the clock** (narrate → measure → derive scene durations); in-app playback **slaves** the animation to `audio.currentTime` (needs the SVG inline — an `<img>` SVG is script-isolated and silent); ambient loop periods must divide the master or `svg-to-video`'s LCM default multiplies the export: **Design only**
- FR-27.14–27.14c **domotion is an EXTERNAL tool the user installs globally, detected at runtime — not bundled** (Decided, NEWS-167/178). Keeps `sharp` (4 unpatched libvips CVEs, no fix available) and a second browser engine out of the shipped closure, so the FR-5.6 entitlements stay shut. Cost: briefing is **off by default**, so nothing on the app's main path may depend on it. Detection **cannot be `which domotion`** — a GUI-launched .app gets no nvm/Homebrew on `PATH`, `PATH` can resolve to an unrelated project's `node_modules/.bin`, and a **version floor is load-bearing** (the globally installed 0.13.3 has no `storyboard` at all). Integration is over the CLI, not the programmatic API.
- FR-27.14d **Shipped (NEWS-178)**: `src/briefing/domotion.ts` finds the install — probe order override → running-node → nvm → Homebrew → system → `PATH` (skipping any `node_modules` entry), version-gated at **0.21.0**, 30 s TTL cache. Verified on the real machine: it finds the *global* 0.13.3 and reports **too-old**, and does NOT pick the unrelated project's copy that `which domotion` resolves to. Route/diagnostics wiring deferred to NEWS-170.
- FR-27.25–27.29 **Shipped (NEWS-167)**: `npm run briefing` renders a reel from the local DB. Reuses `selectForExport` (one definition of "the recent stories"). Markup split from staging so the FR-27.8 guard is testable without a browser; photos are **copied** in, `.bin` cache files get an extension from sniffed magic bytes. **Play length is read from domotion's reported loop, not summed** — transitions add ~2.4 s on a 6-scene reel, and summing would silently truncate an `svg-to-video --duration` export.
- FR-27.15–27.16 rendering is a background job with progress; video additionally needs ffmpeg — now one of **two** external tools: **Design only**
