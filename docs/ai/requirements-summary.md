...
- FR-5.5 signing + notarization: **Partial — config shipped, credentials outstanding.** The full generation recipe now lives in `docs/5-desktop-app.md` (NEWS-21): Developer ID cert via CSR, Team ID, App Store Connect key vs app-specific password, `.p12` export for CI, forward-looking updater keypair, plus a **GitHub Actions secret-name table** and a production-vs-beta section. Key findings recorded there: the Apple credentials are **identical** for production and beta (there is no beta Developer ID cert); `APPLE_API_KEY` is a Key *ID*, not the `.p8`; `APPLE_API_KEY_PATH` is a filesystem path so CI must decode a base64 secret to a file; the release runner must be **macOS** (both current CI jobs are `ubuntu-latest` and cannot sign); and the updater keypair should be **shared** across channels, because the pubkey is compiled into the binary and a per-channel key would strand beta testers.
