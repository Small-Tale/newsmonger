...
- **Release workflow (NEWS-190): shipped, unexercised.** `.github/workflows/release.yml` does build → sign → notarize → `verify-signing.sh` → publish on `macos-latest`. `workflow_dispatch` is a dry run (no inputs, so a branch can never be published); `push: tags v*` publishes, with prerelease derived from a hyphen in the tag, and a guard that the tag's base version matches `tauri.conf.json`/`package.json`. YAML and both shell snippets were validated locally; **the workflow itself has never run** — that needs a push, which is the owner's call. Downloading the resulting `.dmg` is also how NEWS-21's last manual check gets satisfied, since only a real download applies quarantine.
