...
573 requirements across 38 documents (numbered 1–39, with 19 removed and never reused): 559 Shipped · 1 Design only · 1 Deferred · 2 Rejected · 3 Decided · 7 Removed.
...
| [3 — Web UI](../3-ui.md) | Shipped | 75 — 70 Shipped, 5 Removed |
| [4 — CLI, Server, and Storage](../4-cli-server-storage.md) | Shipped | 25 — 25 Shipped |
| [5 — Desktop App (Tauri)](../5-desktop-app.md) | Shipped | 21 — 20 Shipped, 1 Decided |
| [6 — AI Providers](../6-providers.md) | Shipped | 18 — 17 Shipped, 1 Removed |
| [7 — API Keys and Settings Dialog](../7-api-keys.md) | Shipped | 15 — 15 Shipped |
| [8 — Article Images](../8-article-images.md) | Shipped | 20 — 20 Shipped |
| [9 — Subscription Providers](../9-subscription-providers.md) | Shipped | 22 — 21 Shipped, 1 Rejected |
| [10 — New-Item Notifications](../10-notifications.md) | Shipped | 7 — 6 Shipped, 1 Removed |
...
| [20 — First-Run Onboarding](../20-onboarding.md) | Shipped | 16 — 16 Shipped |
| [21 — Export and Feed](../21-export-and-feed.md) | Shipped | 14 — 13 Shipped, 1 Deferred |
| [22 — Topic categories and the filter bar](../22-topic-categories.md) | Shipped | 19 — 19 Shipped |
| [23 — Retries and rate limits](../23-retries-and-rate-limits.md) | Shipped | 7 — 7 Shipped |
| [24 — Topic Discovery](../24-topic-discovery.md) | Shipped | 42 — 42 Shipped |
| [25 — Topic Editing](../25-topic-editing.md) | Shipped | 13 — 13 Shipped |
| [26 — Undo](../26-undo.md) | Shipped | 14 — 14 Shipped |
| [27 — Where Data Is Stored](../27-data-location.md) | Shipped | 14 — 14 Shipped |
| [28 — Demo Capture (the hero and the stills)](../28-demo-capture.md) | Shipped | 24 — 24 Shipped |
| [29 — Story Threads](../29-story-threads.md) | Shipped | 38 — 36 Shipped, 2 Decided |
| [30 — Import (and topic export)](../30-import.md) | Shipped | 16 — 16 Shipped |
| [31 — Deleting in Bulk](../31-bulk-delete.md) | Shipped | 6 — 6 Shipped |
| [32 — When the App Can't Start](../32-startup-failure.md) | Shipped | 8 — 8 Shipped |
| [33 — Getting a Set-Aside Database Back](../33-recovery.md) | Shipped | 7 — 7 Shipped |
| [34 — Letting a New Topic Settle](../34-new-topic-settling.md) | Shipped | 9 — 9 Shipped |
| [35 — Location](../35-location.md) | Shipped | 9 — 9 Shipped |
| [36 — Default Topics from Profiles](../36-profile-topics.md) | Partial | 12 — 11 Shipped, 1 Design only |
| [37 — Topic Pulse](../37-topic-pulse.md) | Shipped | 15 — 15 Shipped |
| [38 — Unified Timeline Product Investigation](../38-unified-timeline-product-investigation.md) | Not stated | none declared |
| [39 — Evidence-Linked Story Thread Briefs](../39-thread-briefs.md) | Shipped | 10 — 10 Shipped |

### Not shipped

Every requirement whose marker says anything other than Shipped.

- [3 — Web UI](../3-ui.md) — **FR-3.46** Removed: NEWS-333
- [3 — Web UI](../3-ui.md) — **FR-3.25** Removed: NEWS-333
- [3 — Web UI](../3-ui.md) — **FR-3.26** Removed: NEWS-333
- [3 — Web UI](../3-ui.md) — **FR-3.27** Removed: NEWS-333
- [3 — Web UI](../3-ui.md) — **FR-3.28** Removed: NEWS-333
- [5 — Desktop App (Tauri)](../5-desktop-app.md) — **FR-5.18** Decided: NEWS-205
- [6 — AI Providers](../6-providers.md) — **FR-6.16** Removed: NEWS-333
- [9 — Subscription Providers](../9-subscription-providers.md) — **FR-9.2** Rejected: the Agent SDK
- [10 — New-Item Notifications](../10-notifications.md) — **FR-10.7** Removed: NEWS-329
...
- [36 — Default Topics from Profiles](../36-profile-topics.md) — **FR-36.11** Design only
...
## [30 — Import](../30-import.md) (NEWS-290 topics, NEWS-289 stories) — **design only, nothing built**

- Written as a design first, because both tickets asked questions ("ask clarifying questions as needed") that were never answered; the doc names every assumption inline so the wrong ones are correctable in one place instead of spread through an implementation. The **topic export** has since shipped (FR-30.2–30.4, NEWS-317) — the half that needed no unanswered question resolved, and the format the two imports read.
- **The gap it identifies is specific**: data leaves the app three ways (story export, backup file, legacy `data.json`) and there is **no additive way in**. Restore replaces everything; the legacy importer runs only into an empty database (NEWS-309 measured it importing nothing, silently). Import is a missing verb, not a missing format.
- **Two questions answered themselves by reading the code**, and both had been posed as open in the feedback notes: the export carries the topic **name** (so a story's topic can be recreated elsewhere) and carries `saved`, but **does not carry `offTopic` at all** — FR-21.2 excludes flagged stories from every export. So "should import bring off-topic flags?" does not arise. Similarly, with no ids or keys in the file, dedup on the recomputed dedupe key is the only available rule, not a choice between several.
- **The load-bearing assumption is FR-30.1**: a topic export is a *shareable list*, not a personal transfer — no ids, no timestamps, no `paused`/`highPriority`. Personal transfer is what backup/restore already is (FR-27.10), and building it twice is how two formats drift apart. Reversing this changes FR-30.2/30.3/30.6 and nothing else.
- **One alternative is refused rather than assumed**: an import never merges into an existing topic. Adopting an incoming `guidance` would overwrite something the user wrote, in bulk, with no diff.
- Worth carrying forward into the implementation: `items` **is** the dedup ledger (FR-2.13), so importing stories quietly narrows what future checks will report. Correct, and surprising enough to be worth a line in the doc.
...
- **An open pane now follows its thread (FR-29.30a, NEWS-293)** — the gap NEWS-282 knowingly left. It turned out to need no new reactive edge, which was the stated reason for leaving it: `loadThread`'s size check already makes a re-call free, so `refreshFeed` just calls it for the expanded card. The work was all in the *refresh* semantics — keep the rows on screen while the bigger thread loads (the `loading` status renders as a placeholder and would blink a correct timeline out), keep them when a background refresh fails (FR-29.35's error pane is for a read someone asked for), and track in-flight separately, because a refresh deliberately leaves the pane `ready` and the status stopped answering that question.
...
- **The capture must photograph the app, not the machine holding the camera (NEWS-315).** `--demo` now answers `/api/providers` and `/api/keys` from a fixture, injected as `createApp`'s `probe` / `demoKeys`. Those were the last two things in demo mode that read the capturing laptop: `settings-source.png` named whichever provider the owner was signed in to, its key rows changed with `ANTHROPIC_API_KEY`, and the sentence beneath them named the OS's credential store. **The ticket found one of the two; the second turned up by looking at the actual image.**
- Deterministic *by accident* until NEWS-308 — the status line rendered blank on the default `auto` setting, so there was nothing to vary. A property nothing asserts can hold for a year and then be broken by a change to an unrelated file, which is the argument for `tests/unit/demo-determinism.test.ts` asserting it directly (same request, with and without an API key exported, plus the opposite direction so the equality is not vacuous).
- `--ai-test` deliberately keeps the **real** probe: the E2E suite asserts on provider availability, and a canned answer would assert nothing.
...
- FR-24.1–24.18, NEWS-116. Approved shape: **two doors into one result list** — a free-text box (FR-24.3) and a grid of the NEWS-97 sections (FR-24.2) — with a **keep/skip tuner as the depth control**, not a third door (FR-24.5–24.9). Reached from beside the add-topic field and from onboarding's Topics step, replacing its six hard-coded chips.
...
- **docs/22-topic-categories.md** (NEWS-97) — topic categories. FR-22.1–22.7 code-side taxonomy, slug-not-label storage, retire-not-delete, most-specific label resolution, "no subcategory" as a rendered *Other*, topic `category`/`subcategory`/`categorySource` + manual-override route (schema v2 migration): **Shipped**. FR-22.8 auto-classification on the first check, model slugs validated against the live taxonomy: **Shipped**. FR-22.9 section label on its own line with the full path (NEWS-111), FR-22.10–22.12 two-row newspaper-style filter bar, server-side filtering with `uncategorized`/`other` sentinels, ephemeral filter state; FR-22.13–22.15 (NEWS-114) empty options hidden, no subsection row below two options, active selection always kept: **Shipped**
  ...
  **The taxonomy is 20 sections wide, not 11** (NEWS-388) — Money, Media, Living, Law & Justice and Transport are new; Environment, Food & Drink, Travel and Education were promoted out of Science, Culture and Society. Nothing was deleted: nine moved rows are `retired` in place and six widened labels keep their original slug, so no stored classification lost its label. **The "eleven is a ceiling because an unused pill costs bar space" rationale was already false when it was written down** — FR-22.13 (NEWS-114) stopped rendering unpopulated sections three tickets later, and the comment survived until NEWS-392 read it against the code. The size budget that *is* real is the classifier's option list: every section and subcategory goes into the check prompt as a choice.
...
- FR-3.25–3.26 in-app diagnostics — recent-checks list + copy-diagnostics bundle, with topic names redacted by default and the endpoint reported as set/not-set rather than by URL (NEWS-88): **Removed, NEWS-333**. This was the app's bug-report path, so a user hitting a problem now has nothing to paste; the run history it read is still stored (FR-25.7) and everything it assembled is still behind `/api/state`, so rebuilding it would be a rendering job.
...
- **Updater custom-command ACL corrected (NEWS-447).** The loopback-hosted client invokes Newsmonger's `get_pending_update`, `check_for_update`, and `install_update` wrappers, not the updater plugin commands directly. `updater:default` therefore never authorized them: `build.rs` now declares all three in the application manifest so Tauri generates their `allow-*` permissions, and the loopback capability grants those permissions explicitly. A unit test cross-checks the handler, manifest, and grants as sets so adding a fourth wrapper cannot silently recreate the release-only ACL failure.
...
- FR-6.16 **Effort comparison** (Settings → App → Diagnostics): **Removed, NEWS-333**, with the section that held it. FR-6.15's per-run effort recording is untouched — which is the part NEWS-19 was blocked on — so the view could be rebuilt from `runs` alone.
- FR-10.7 **Send a test notification** button (Settings → App): shipped in NEWS-260, **removed in NEWS-329** on request. Worth remembering why it existed, because the reason did not go away: notifications are otherwise unobservable on demand, and on macOS an app appears in System Settings → Notifications only once it has *delivered* one — so this was the only way to make Newsmonger's entry exist without waiting for a real story to arrive while the window was in the background.
