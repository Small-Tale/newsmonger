...
- **FR-6.13** *(Shipped, NEWS-189)* **Effort is a setting** — a `<select>` in the Source block, `effort` in `Settings`, seeded by `--effort` / `NEWSMONGER_EFFORT`. Levels: `low`, `medium`, `high`, `xhigh`, `max`, plus **"Provider default"** (`''`), which is the default — so behaviour is unchanged until someone chooses.
  ...
  **Every provider takes one** except the test-only `mock`. `providerTakesEffort` in `src/ai/types.ts` is the single list, replacing a hardcoded `provider === 'anthropic'` in the UI.
  ...
  The Codex key name was **verified, not guessed**: with `--strict-config`, Codex accepts `model_reasoning_effort` and rejects an invented key as an "unknown configuration field". Worth knowing that **without `--strict-config` an unknown key is swallowed in silence** — the check that makes this verifiable is not the one a normal invocation performs. There is no OpenAI key on the development machine, so two errors were captured through the Codex CLI's ChatGPT-subscription path and pinned as test fixtures: a `400` naming `[reasoning.effort]` in its message (which is what the match relies on), and a `400` about an unsupported *model* (which confirms the narrowness earns its keep — dropping effort would not make an unsupported model supported). **Still unverified is the case the retry exists for**: a genuine "this model does not do reasoning" refusal. It cannot be reached through Codex at all — a ChatGPT subscription offers reasoning models only, and refuses every non-reasoning one *before* the parameter is evaluated (checked against `gpt-4o`, `gpt-4o-mini`, `gpt-4.1`, `gpt-5-chat-latest`, `gpt-5.1-chat-latest`). Closing it needs an OpenAI API key. What Codex *does* confirm is the happy path: a valid level is accepted and applied (`reasoning effort: high` in its own run header). That residual uncertainty is why the code asserts nothing about which models qualify.
