...
- **FR-5.3** *(Shipped, verified on macOS, Windows and Linux)* `npm run tauri:build` produces a self-contained app: a real Node binary ships beside the app executable as an `externalBin` sidecar (`newsmonger-node`), and the tsup server bundle plus its client assets and full dependency tree are staged into `resources/server/`. The release shell resolves both and spawns the server exactly as dev mode does — same readiness-line watch, same navigation, same shutdown. `scripts/build-sidecar.sh` produces all of it and runs as `beforeBuildCommand`, so `tauri build` is self-contained. App icons live in `src-tauri/icons/`, generated from `assets/logo-full-bleed.svg` with `npx tauri icon` (required even for dev compilation). Regenerate from that source rather than editing the PNGs (see `docs/3-ui.md` for why the *full-bleed* variant is the source, which is a deliberate choice against the macOS convention) — and delete the `android/` and `ios/` directories it also writes, since this is a desktop-only app and ~30 unused mobile icons is dead weight in the tree.
  ...
  **Linux is now verified end to end too** (NEWS-20), and in CI rather than a VM — `.github/workflows/linux-bundle-diagnostic.yml`, dispatch-only, installs a *published* bundle on a GitHub runner. Both formats pass, and both matter because they resolve resources differently: the `.deb` via `apt install ./file.deb` (which also checks the package's declared dependencies), the `.AppImage` extracted, since runners have no FUSE. Each one spawns its sidecar, prints the readiness line, serves the shell HTML and `/static/*` from the installed location, opens a window, **paints** it, and closes leaving no orphaned `newsmonger-node`.

  Docker had been recorded as unable to do this, for having no GTK/WebKit stack and no display. Both are fixable — `apt` provides one and Xvfb the other — and a GitHub runner is simply a Linux box that already has apt, so it was cheaper than either a container or another VM. The earlier blocked reason was more final than the facts warranted.

  **A headless run cannot prove the window looks right**, so it asserts the frame is not uniformly blank — a dead webview still opens a window, and "it launched" is exactly the assertion that would pass while the app was unusable — and keeps the screenshot as an artifact for one human look. It also **closes the window** rather than killing the process, since killing it would void the orphaned-sidecar check. `WEBKIT_DISABLE_COMPOSITING_MODE` and `WEBKIT_DISABLE_DMABUF_RENDERER` are set because WebKitGTK assumes a GPU no runner has; that is a property of the environment, not something the product needs.

  Three wrong binaries were tried before the right one, all my own error rather than the app's: a hardcoded `/usr/bin/newsmonger` (the bundle installs `Newsmonger`), then the first `/usr/bin/` entry in the package — which is the **sidecar**, a Node binary that with no script exits 0 in silence and is indistinguishable from an app crashing on startup. The `.desktop` entry's `Exec=` is the authority, being how a Linux desktop itself launches the app.
  ...
  **Both Linux triples now build their sidecar for real** (NEWS-20): `bash scripts/verify-sidecar-linux.sh` runs `build-sidecar.sh` inside a Linux container for `x86_64-unknown-linux-gnu` and `aarch64-unknown-linux-gnu`, and both produce a correct ELF and pass the isolated boot check, serving all seven static assets.
  ...
  What that still does **not** cover, and cannot from a container: building the Tauri bundle itself (`.deb`/`.AppImage`), and launching the packaged app to watch the sidecar spawn and exit cleanly. Those need the GTK/WebKit stack and a desktop session — a VM. Windows is untouched on all counts.
...
- **FR-5.5** *(Shipped, verified on macOS)* Release bundles are signed with a **Developer ID Application** certificate and notarized. Nothing identity-specific is committed: Tauri reads `APPLE_SIGNING_IDENTITY` (or `APPLE_CERTIFICATE` + `APPLE_CERTIFICATE_PASSWORD` in CI) and, for notarization, **`APPLE_ID` + `APPLE_PASSWORD` + `APPLE_TEAM_ID`** — an **app-specific password**, which is the route this project uses. The App Store Connect key trio (`APPLE_API_ISSUER` + `APPLE_API_KEY` + `APPLE_API_KEY_PATH`) is the documented migration target for when the credential should stop being tied to one person's Apple ID.

  **Marked Partial — "credentials outstanding" — until NEWS-343 checked.** They have not been outstanding for some time: every `Build Tauri Signed` job in `v0.2.0-beta.15`'s release run succeeded, and `scripts/verify-released-dmg.sh` has confirmed stapling and a `source=Notarized Developer ID` assessment on published artifacts since `v0.2.0-beta.7`. The App Store Connect migration above is a *preference* about whose Apple ID the credential belongs to, not an unmet requirement — leaving the status at Partial made the generated summary understate what actually ships.
...
- **FR-5.14** *(Shipped, NEWS-89; corrected NEWS-447)* Three commands make up the whole bridge: `get_pending_update` (read the parked version), `check_for_update` (ask now, from Settings), and `install_update` (download + install). All three are **`#[cfg(not(debug_assertions))]`-guarded**, so `tauri dev` never tries to update an unsigned binary whose version is whatever `tauri.conf.json` last said. In a debug build they compile to `Ok(None)` / `Ok(())`. Because the window navigates from bundled content to the loopback server, these **application commands need their own ACL grants**: `build.rs` declares the command manifest so Tauri generates `allow-*` permissions, and `capabilities/default.json` grants all three to the remote loopback capability. `updater:default` alone covers only the plugin's commands; it does not authorize Newsmonger's wrappers.
...
- **FR-5.16** *(Shipped, NEWS-89; hardened NEWS-446)* **Settings → App → "Check for updates"** is the manual path, and it is the only one that reports *negative* results — "Newsmonger is up to date." or "Could not check for updates: [native reason]." The startup check stays silent on both, deliberately: a failed background check is not worth a banner, but silence in answer to a button press reads as a bug. A native check retries once before failing; GitHub's endpoint redirects to release-asset storage, so one logical check crosses multiple network hops and should survive one transient DNS/TLS/proxy failure. If both attempts fail, the command preserves both native errors rather than flattening every transport, target and manifest problem into the same undiagnosable sentence. The control is **desktop-only** (`isTauri()`), since the browser build has no app binary to replace.
