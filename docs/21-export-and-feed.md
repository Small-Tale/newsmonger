...
- **FR-21.11** *(Shipped, NEWS-161)* The button is **filled, and carries a `download` icon**. It shipped outlined with `share-2` — three linked circles, which name handing something to another person or app. This writes a file to disk. It is also the only action on the Data tab, and an outlined button there read as an afterthought.
  ...
  Its icon sits on the label's **centre line**: the row's `.btn` is a flex row, without which a 15px glyph is an inline box on the 13px text's baseline and rides visibly high. All three are measured in `tests/e2e/export.spec.ts` — centre offset under 1.5px, the glyph asserted by *what it is made of* (a `polyline` arrowhead and no `circle`s, which survives a resize or recolour), and the fill compared against `--panel` rather than a hex so it holds in both themes.
