...
describe('Store', () => {
  ...
  it('marks a topic high-priority and back (NEWS-56)', () => {
    ...
    expect(tmpStore(store.dataDir).getTopic(topic.id)?.highPriority).toBe(true); // persisted
  ...
  it('flags a story off-topic and lists flagged titles for the prompt (NEWS-61)', () => {
    ...
    expect(tmpStore(store.dataDir).offTopicTitlesForTopic(topic.id)).toHaveLength(2);
