...
- **kerf client rules** (enforced by `eslint-plugin-kerfjs`, documented in `docs/3-ui.md`): state in `defineStore`/signals; events via `delegate()` with `data-*` attributes (never `addEventListener`/inline handlers); `data-key` on list rows; `.map()` not `each()` for static structural arrays. Plus two hard-won structural rules: keep `each()` containers structurally stable (don't conditionally swap the container element), and wrap conditional siblings (banners) in an always-present container so they can't shift unkeyed siblings.
