...
- **E2E tests** (`tests/e2e/**/*.spec.ts`): Playwright, serial (`workers: 1`), one shared server started by `playwright.config.ts` with `--ai-test` and a pid-scoped temp data dir. Tests build on each other's state (documented at the top of the spec).
