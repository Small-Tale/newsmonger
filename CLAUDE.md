...
- **E2E tests** (`tests/e2e/**/*.spec.ts`): Playwright, serial (`workers: 1`), one shared server started by `playwright.config.ts` with `--ai-test` and a pid-scoped temp data dir. Tests build on each other's state **within a file** (documented at the top of the spec); across files they must not — every file calls `resetSharedState` in its own `beforeAll` (NEWS-313), and a file that needs a topic with stories says so with `seedCheckedTopic` rather than inheriting one (NEWS-322).
